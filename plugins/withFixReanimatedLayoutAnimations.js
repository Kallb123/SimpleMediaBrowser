...
// LayoutAnimationsProxy.cpp and LayoutAnimationsUtils.cpp.  React Native 0.79
// removed that field; the struct now exposes only `parentTag` (a Tag/int)
// instead of the full ShadowView.
//
// This plugin patches both files during prebuild so they compile cleanly
// against React Native 0.79:
//
//   Fix 1 – .parentShadowView.tag → .parentTag
//            Covers all direct tag-access sites (parseRemoveMutations,
//            updateIndexForMutation, maybeUpdateWindowDimensions,
//            Node::applyMutationToIndices, etc.)
//
...
function patchParentShadowView(filePath, label) {
  if (!fs.existsSync(filePath)) {
    console.warn(
      `[withFixReanimatedLayoutAnimations] ${label} not found — skipping.`
    );
    return;
  }
  ...
  if (
    ...
    !contents.includes('mutation.parentShadowView')
  ) {
    console.log(
      `[withFixReanimatedLayoutAnimations] ${label}: No parentShadowView code patterns found — no patch needed.`
    );
    return;
  }
  ...
  console.log(
    `[withFixReanimatedLayoutAnimations] Patched ${label} for RN 0.79 compatibility.`
  );
}
...
function withFixReanimatedLayoutAnimations(config) {
  return withDangerousMod(config, [
    'android',
    (config) => {
      const layoutAnimationsDir = path.join(
      ...
      patchParentShadowView(
        path.join(layoutAnimationsDir, 'LayoutAnimationsProxy.cpp'),
        'LayoutAnimationsProxy.cpp'
      );

      patchParentShadowView(
        path.join(layoutAnimationsDir, 'LayoutAnimationsUtils.cpp'),
