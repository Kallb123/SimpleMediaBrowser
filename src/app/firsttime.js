...
export default function FirstTime() {
    ...
    return (<ScrollView contentContainerStyle={styles.contentContainer}>
      ...
      {sourceAdded && (<ThemedText style={styles.addedNote} accessibilityLabel="Source added successfully. You can add more in Settings later.">
          ✓ Source added. You can add more in Settings later.
        </ThemedText>)}
      <ThemedView style={styles.titleContainer}>
        <ThemedText>Settings password (optional):</ThemedText>
        <ThemedTextInput onChangeText={onChangePassword} value={password ?? ""} placeholder="Leave blank for no password" keyboardType="default" secureTextEntry={true}/>
      </ThemedView>
      <ThemedView style={styles.titleContainer}>
        <ThemedText>Generate video thumbnails:</ThemedText>
        <Switch value={enableThumbnailGeneration} onValueChange={setEnableThumbnailGenerationLocal}/>
      </ThemedView>
      <ThemedText style={styles.addedNote}>
        Off by default. You can change this later in Settings.
      </ThemedText>
      <ThemedView style={styles.titleContainer}>
        <Button title="Finished, Go Home" onPress={finishedGoHome}/>
      </ThemedView>
