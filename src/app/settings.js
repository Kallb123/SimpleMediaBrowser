...
import { selectDataSource, selectMediaSources, selectMediaStructure, selectPassword, selectViewOrientation, selectViewScale, setDataSource, setMediaStructure, setPassword, setViewOrientation, setViewScale, removeMediaSource, selectTmdbApiKey, setTmdbApiKey, selectDefaultPage, setDefaultPage, selectEnablePosterFetching, setEnablePosterFetching, selectEnableThumbnailGeneration, setEnableThumbnailGeneration } from '@/store/settingsReducer';
...
class SettingsErrorBoundary extends React.Component {
    ...
    render() {
        if (this.state.hasError) {
            return (<ThemedView style={styles.crashContainer}>
...
export default function SettingsPrompt() {
    ...
    const [password, setLocalPassword] = useState(null);
    const [tmdbApiKey, setLocalTmdbApiKey] = useState(null);
    const [dataSource, setLocalDataSource] = useState("");
    const [mediaStructure, setLocalMediaStructure] = useState("");
    const [structureDescription, setStructureDescription] = useState("");
    const [viewOrientation, setLocalViewOrientation] = useState("");
    const [viewScale, setLocalViewScale] = useState(2);
    const [defaultPage, setLocalDefaultPage] = useState("home");
    ...
    const safeDecodeUri = useCallback((uri) => {
        try {
    ...
    const safeMediaSources = useMemo(() => {
        try {
            ...
            const sanitised = mediaSources.filter((source) => {
            ...
            logger.error('Settings', 'Failed to sanitise mediaSources, using empty list', e);
            return [];
        }
    }, [mediaSources]);
    useEffect(() => {
        try {
            ...
            logger.error('Settings', 'Exception while logging settings mount state', e);
        }
    ...
    useEffect(() => {
        try {
            ...
            if (dataSourceRef.current && dataSourceIndex >= 0) {
                dataSourceRef.current.selectIndex(dataSourceIndex);
            }
            ...
            if (mediaStructureRef.current && mediaStructureIndex >= 0) {
                mediaStructureRef.current.selectIndex(mediaStructureIndex);
            }
            ...
            if (viewOrientationRef.current && viewOrientationIndex >= 0) {
                viewOrientationRef.current.selectIndex(viewOrientationIndex);
            }
            ...
            if (defaultPageRef.current && defaultPageIndex >= 0) {
                defaultPageRef.current.selectIndex(defaultPageIndex);
            }
            ...
            logger.error('Settings', 'Exception while syncing local settings state', e);
        }
    ...
    const save = () => {
        try {
            ...
            logger.error('Settings', 'Exception while saving settings', e);
        }
    };
    const deleteSource = useCallback((uri) => {
        try {
            logger.log('Settings', `Removing media source: ${uri}`);
            dispatch(removeMediaSource(uri));
        }
        catch (e) {
            logger.error('Settings', 'Exception while removing media source', e);
        }
    }, [dispatch]);
    const scanNow = useCallback(async () => {
        try {
            ...
            logger.error('Settings', 'Exception while scanning sources', e);
        }
        finally {
            setScanning(false);
        }
    }, [safeMediaSources]);
    const handleUIScaleChange = (value) => {
        try {
            setLocalViewScale(11 - value);
        }
        catch (e) {
            logger.error('Settings', 'Exception while applying UI scale change', e);
        }
    };
    ...
    <ScrollView style={containerStyle} contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
      ...
      <View style={styles.section}>
        ...
        <View style={styles.row}>
          ...
          <ThemedTextInput onChangeText={setLocalPassword} value={password ?? ""} placeholder="Settings password" keyboardType="default" secureTextEntry={true}/>
        </View>
      </View>
      ...
      <View style={styles.section}>
        ...
        {safeMediaSources.length === 0 && (<ThemedText style={styles.emptyText}>No sources added yet.</ThemedText>)}
        {safeMediaSources.map((source) => (<View key={source.uri} style={styles.sourceRow}>
      ...
      <View style={styles.section}>
        ...
        <View style={styles.row}>
          ...
          <SelectDropdown ref={dataSourceRef} data={dataSourceOptions} defaultValue={dataSourceOptions.find(o => o.id === settingsDataSource)} onSelect={(selectedItem) => {
            try {
                setLocalDataSource(selectedItem.id);
            }
            catch (e) {
                logger.error('Settings', 'Exception while selecting data source', e);
            }
        }} renderButton={(selectedItem, isOpened) => (<View style={[styles.dropdownButtonStyle, { backgroundColor: dropdownBg }]}>
                <Text style={[styles.dropdownButtonTxtStyle, { color: theme.text }]}>
                  {(selectedItem && selectedItem.label) || 'Please select...'}
                </Text>
                <Text>{isOpened ? "🔼" : "🔽"}</Text>
              </View>)} renderItem={(item, index, isSelected) => (<View style={{ ...styles.dropdownItemStyle, backgroundColor: isSelected ? dropdownSelectedBg : dropdownBg }}>
                <Text style={[styles.dropdownItemTxtStyle, { color: theme.text }]}>{item.label}</Text>
              </View>)} showsVerticalScrollIndicator={false} dropdownStyle={[styles.dropdownMenuStyle, { backgroundColor: dropdownBg }]}/>
        </View>

        <ThemedText style={styles.rowLabel}>TMDB API Key:</ThemedText>
        <ThemedTextInput onChangeText={setLocalTmdbApiKey} value={tmdbApiKey ?? ""} placeholder="Paste your TMDB API key" keyboardType="default" secureTextEntry={false}/>
        ...
        <View style={styles.row}>
          ...
          <Switch value={enablePosterFetching} onValueChange={(value) => {
            try {
                setLocalEnablePosterFetching(value);
            }
            catch (e) {
                logger.error('Settings', 'Exception while toggling poster fetching', e);
            }
        }}/>
        </View>
        <ThemedText style={styles.emptyText}>
        ...
        <View style={styles.row}>
          ...
          <Switch value={enableThumbnailGeneration} onValueChange={(value) => {
            try {
                setLocalEnableThumbnailGeneration(value);
            }
            catch (e) {
                logger.error('Settings', 'Exception while toggling thumbnail generation', e);
            }
        }}/>
        </View>
        <ThemedText style={styles.emptyText}>
      ...
      <View style={styles.section}>
        ...
        <View style={styles.row}>
          ...
          <SelectDropdown ref={mediaStructureRef} data={viewTypeOptions} defaultValue={viewTypeOptions.find(o => o.id === settingsMediaStructure)} onSelect={(selectedItem) => {
            try {
                if (selectedItem.title)
                    setStructureDescription(selectedItem.title);
                setLocalMediaStructure(selectedItem.id);
            }
            catch (e) {
                logger.error('Settings', 'Exception while selecting media structure', e);
            }
        }} renderButton={(selectedItem, isOpened) => (<View style={[styles.dropdownButtonStyle, { backgroundColor: dropdownBg }]}>
                <Text style={[styles.dropdownButtonTxtStyle, { color: theme.text }]}>
                  {(selectedItem && selectedItem.label) || 'Please select...'}
        ...
        {structureDescription && (<ThemedText style={styles.emptyText}>{structureDescription}</ThemedText>)}

        <View style={styles.row}>
          <ThemedText style={styles.rowLabel}>Interface type:</ThemedText>
          <SelectDropdown ref={viewOrientationRef} data={uiTypeOptions} defaultValue={uiTypeOptions.find(o => o.id === settingsViewOrientation)} onSelect={(selectedItem) => {
            try {
                setLocalViewOrientation(selectedItem.id);
            }
            catch (e) {
                logger.error('Settings', 'Exception while selecting view orientation', e);
            }
        }} renderButton={(selectedItem, isOpened) => (<View style={[styles.dropdownButtonStyle, { backgroundColor: dropdownBg }]}>
                <Text style={[styles.dropdownButtonTxtStyle, { color: theme.text }]}>
                  {(selectedItem && selectedItem.label) || 'Please select...'}
        ...
        <View style={styles.row}>
          ...
          <Slider style={styles.slider} minimumValue={1} maximumValue={10} step={1} value={11 - settingsViewScale} onSlidingComplete={handleUIScaleChange} minimumTrackTintColor={colorScheme === 'dark' ? '#ECEDEE' : '#11181C'} maximumTrackTintColor={colorScheme === 'dark' ? '#687076' : '#9BA1A6'}/>
        </View>

        <View style={styles.row}>
          <ThemedText style={styles.rowLabel}>Default page:</ThemedText>
          <SelectDropdown ref={defaultPageRef} data={defaultPageOptions} defaultValue={defaultPageOptions.find(o => o.id === settingsDefaultPage)} onSelect={(selectedItem) => {
            try {
                setLocalDefaultPage(selectedItem.id);
            }
            catch (e) {
                logger.error('Settings', 'Exception while selecting default page', e);
            }
        }} renderButton={(selectedItem, isOpened) => (<View style={[styles.dropdownButtonStyle, { backgroundColor: dropdownBg }]}>
                <Text style={[styles.dropdownButtonTxtStyle, { color: theme.text }]}>
                  {(selectedItem && selectedItem.label) || 'Please select...'}
