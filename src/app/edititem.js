...
import { ActivityIndicator, Button, FlatList, Linking, ScrollView, StyleSheet, TouchableOpacity, View, } from 'react-native';
...
import { selectMediaLibrary, selectMediaOverrides, selectMovies, setMediaOverride, updateShowMetadata, updateMovieMetadata, } from '@/store/libraryReducer';
 ...
 */
async function copyPickedPoster(sourceUri, key) {
    await ensurePostersDir();
...
export default function EditItemScreen() {
    ...
    const [activePosterUri, setActivePosterUri] = useState(existingOverride.poster || currentLibraryPoster || '');
    ...
    const [matchApplied, setMatchApplied] = useState(null);
    ...
    const itemTypeLabel = itemType === 'show' ? 'TV Show' : itemType === 'movie' ? 'Movie' : 'Episode';
      ...
      <ScrollView style={styles.container} contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        ...
        <View style={styles.section}>
          ...
          <View style={styles.field}>
            ...
            <ThemedTextInput value={titleInput} onChangeText={setTitleInput} placeholder="Override display title"/>
          </View>
          <View style={styles.field}>
            <ThemedText style={styles.label}>Sort Title</ThemedText>
            <ThemedTextInput value={sortTitleInput} onChangeText={setSortTitleInput} placeholder="Override sort order (e.g. 'Dark Knight, The')"/>
        ...
        {(itemType === 'show' || itemType === 'movie') && (<View style={styles.section}>
            <ThemedText type="defaultSemiBold" style={styles.sectionTitle}>Poster Override</ThemedText>

            {activePosterUri ? (<Image source={{ uri: activePosterUri }} style={styles.posterPreview} contentFit="contain"/>) : (<View style={styles.posterPlaceholder}>
            ...
            <View style={styles.posterButtonRow}>
              <TouchableOpacity style={styles.posterButton} onPress={handleGoogleImageSearch}>
                <ThemedText style={styles.posterButtonText}>🔍 Google Images</ThemedText>
              </TouchableOpacity>

              <TouchableOpacity style={styles.posterButton} onPress={handleBrowseLocally} disabled={browsingLocally}>
        ...
        {canRematch && (<View style={styles.section}>
            <ThemedText type="defaultSemiBold" style={styles.sectionTitle}>
              Search TMDB for Poster
            </ThemedText>

            <View style={styles.searchRow}>
              <ThemedTextInput value={searchQuery} onChangeText={setSearchQuery} placeholder="Search TMDB…" style={styles.searchInput} onSubmitEditing={handleSearch} returnKeyType="search"/>
              <TouchableOpacity style={styles.searchButton} onPress={handleSearch} disabled={searching || applyingMatch}>
            ...
            {applyingMatch && (<ThemedText style={styles.hint}>Applying match…</ThemedText>)}

            {searchResults.length > 0 && (<FlatList data={searchResults} keyExtractor={(item) => String(item.id)} scrollEnabled={false} renderItem={({ item }) => {
                    const title = item.title ?? item.name ?? '';
                    const year = (item.release_date ?? item.first_air_date ?? '').substring(0, 4);
                    const isSelected = matchApplied === item.id;
                    return (<TouchableOpacity style={[styles.resultRow, isSelected && styles.resultRowSelected]} onPress={() => handleSelectResult(item)} disabled={applyingMatch}>
                      {item.poster_path ? (<Image source={{ uri: POSTER_THUMB_URL + item.poster_path }} style={styles.resultPoster} contentFit="cover"/>) : (<View style={[styles.resultPoster, styles.resultPosterPlaceholder]}>
                          <ThemedText style={styles.placeholderIcon}>🎬</ThemedText>
                        </View>)}
                      <View style={styles.resultInfo}>
                        <ThemedText style={styles.resultTitle}>{title}</ThemedText>
                        {year !== '' && (<ThemedText style={styles.resultYear}>{year}</ThemedText>)}
                        {isSelected && (<ThemedText style={styles.selectedLabel}>✔ Matched</ThemedText>)}
                      </View>
                    </TouchableOpacity>);
                }}/>)}
          </View>)}

        {!tmdbApiKey && (itemType === 'show' || itemType === 'movie') && (<View style={styles.section}>
            <ThemedText style={styles.hint}>
