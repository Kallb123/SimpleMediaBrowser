...
export default function LogsScreen() {
    ...
    return (<ThemedView style={styles.container}>
            ...
            <View style={[styles.toolbar, { backgroundColor: isDark ? '#1E1E1E' : '#F0F0F0' }]}>
                ...
                <TouchableOpacity onPress={() => setAutoScroll((v) => !v)} style={[styles.toolbarButton, autoScroll && styles.toolbarButtonActive]}>
            ...
            <View style={[styles.filterBar, { backgroundColor: isDark ? '#1E1E1E' : '#F0F0F0' }]}>
                {filterButtons.map(({ label, value }) => (<TouchableOpacity key={value} onPress={() => setFilter(value)} style={[styles.filterButton, filter === value && styles.filterButtonActive]}>
            ...
            }]} placeholder="Filter by text…" placeholderTextColor={isDark ? '#888' : '#999'} value={searchText} onChangeText={setSearchText} autoCapitalize="none" autoCorrect={false} clearButtonMode="while-editing"/>
            </View>
            ...
            {/* Log output */}
            <ScrollView ref={scrollRef} style={styles.scroll} contentContainerStyle={styles.scrollContent} onScrollBeginDrag={() => setAutoScroll(false)}>
                {filteredLines.length === 0 ? (<ThemedText style={styles.emptyText}>
                        {lines.length === 0
                ? 'No log entries yet.'
                : 'No log entries match the current filter.'}
                    </ThemedText>) : (filteredLines.map((line, i) => (<ThemedText key={`${i}-${line.substring(0, 20)}`} style={[styles.logLine, { color: lineColor(line) }]} selectable>
                            {line}
                        </ThemedText>)))}
            </ScrollView>
        </ThemedView>);
