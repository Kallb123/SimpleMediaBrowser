 ...
 */
async function copyLocalPoster(sourceUri, key) {
...
export class FileScanner {
    ...
    async scanAllSources(sources) {
        ...
        try {
            ...
            for (const src of tvSources) {
                ...
                posterMap.forEach((uri, key) => { if (!tvPosterMap.has(key))
                    tvPosterMap.set(key, uri); });
            }
            ...
            for (const src of movieSources) {
                ...
                posterMap.forEach((uri, key) => { if (!moviePosterMap.has(key))
                    moviePosterMap.set(key, uri); });
            }
            ...
            if (enableThumbnailGeneration) {
                ...
                if (uncached.length > 0) {
                    ...
                    await Promise.allSettled(uncached.map(async (media) => {
    ...
    async scanFolder(directory) {
        ...
        const { files: allMediaFiles, posterMap } = await this.collectAllMediaFiles(directory, new Semaphore(MAX_CONCURRENT_DIR_READS), { filesFound: 0 }, 'tv');
        const library = this.buildLibrary(allMediaFiles, posterMap);
        store.dispatch(setMediaLibrary(library));
        return filtered;
    }
    // ─── Recursive collection ────────────────────────────────────────────────
    async collectAllMediaFiles(rootDirectory, dirSemaphore, progress, sourceType = 'tv') {
        const result = [];
    ...
    async recursiveCollect(directory, relativePathParts, result, posterMap, depth, dirSemaphore, progress, streamState) {
        ...
        for (let i = 0; i < contents.length; i++) {
            ...
            if (this.isMediaFile(filename)) {
                ...
                if (NON_DIRECTORY_EXTENSIONS.has(ext))
                    continue;
        ...
        await Promise.allSettled(subdirs.map(({ uri, pathParts }) => this.recursiveCollect(uri, pathParts, result, posterMap, depth + 1, dirSemaphore, progress, streamState)));
    }
     ...
     */
    addToStreamBatch(file, posterMap, streamState) {
     ...
     */
    flushStreamBatch(streamState, posterMap) {
    ...
    async generateThumbnail(videoUri) {
        ...
        try {
            ...
            const releasable = player;
            releasable.release?.();
            releasable.destroy?.();
        }
    }
    // ─── Movie list building ─────────────────────────────────────────────────
    buildMovieList(files, posterMap) {
    ...
    parseFilename(filename) {
        ...
        const sxxMatch = withoutExt.match(/^(.*?)[\.\s_-]+\b[Ss](\d{1,2})[Ee](\d{1,3})\b(?:[\.\s_-]+(.*))?$/);
        if (sxxMatch) {
            return {
        ...
        const nxnnMatch = withoutExt.match(/^(.*?)[\.\s_-]+\b(\d{1,2})x(\d{1,3})\b(?:[\.\s_-]+(.*))?$/);
        if (nxnnMatch) {
            return {
