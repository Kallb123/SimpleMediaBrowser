...
import { FileScanner, thumbnailCache } from '@/scripts/FileScanner';
import { logger } from '@/scripts/Logger';
import { useEditMode } from '@/contexts/EditModeContext';
// ── Helper: pick a representative thumbnail for a show folder ─────────────────
function pickShowThumbnail(library, showName) {
...
function movieSortKey(movie, overrides) {
 ...
 */
function compareEpisodes(a, b, overrides) {
...
const NATURAL_SORT_OPTS = { numeric: true, sensitivity: 'base' };
/** Returns the effective display label for an episode, applying overrides to the title portion. */
function episodeDisplayLabel(ep, overrides, prefix) {
...
function movieDisplayLabel(movie, overrides) {
    return overrides[`movie:${movie.path}`]?.title ?? movie.title ?? movie.filename;
}
function buildDisplayItems(library, movies, viewType, navStack, overrides, navigateInto) {
    switch (viewType) {
        case 'flat': {
            ...
            const sortedMovies = [...movies].sort((a, b) => movieSortKey(a, overrides).localeCompare(movieSortKey(b, overrides), undefined, NATURAL_SORT_OPTS));
        ...
        case 'show': {
            if (navStack.length === 0) {
                ...
                const showFolders = Object.keys(library)
                ...
                const sortedMovies = [...movies].sort((a, b) => movieSortKey(a, overrides).localeCompare(movieSortKey(b, overrides), undefined, NATURAL_SORT_OPTS));
                const movieItems = sortedMovies.map((movie) => ({
                    kind: 'file',
                    label: movieDisplayLabel(movie, overrides),
                    key: movie.path,
                    thumbnailUri: thumbnailCache.get(movie.path),
...
function mapScaleToColumns(viewScale, minColumns, maxColumns) {
...
export function MediaBrowserScreen({ mediaFilter }) {
    ...
    // Apply filter
    const mediaLibrary = mediaFilter === 'movies' ? {} : allLibrary;
    const movies = mediaFilter === 'tv' ? [] : allMovies;
    ...
    const displayItems = useMemo(() => buildDisplayItems(mediaLibrary, movies, viewType, navStack, mediaOverrides, navigateInto), 
    ...
    const breadcrumb = [
        rootLabel,
        ...navStack.map((n) => n.label),
    ].join(' › ');
    /** Open the edit screen for a given display item. */
    const openEditScreen = useCallback((item) => {
        let itemType;
        let itemKey;
    ...
    return (<View style={styles.container}>
      {mediaSources.length > 0 && hasLibraryContent ? (<ThemedView style={styles.listContainer}>
          {/* Scan progress banner – shown at the top of the grid while any scan phase is active */}
          {isScanning && scanProgress.phase !== 'idle' && (<View style={styles.scanBanner}>
              ...
              {scanProgress.phase === 'thumbnails' && (<View style={styles.scanProgressTrack}>
                  <View style={[
                        styles.scanProgressFill,
                        {
                            width: `${Math.round((scanProgress.thumbnailsDone / Math.max(1, scanProgress.thumbnailsTotal)) * 100)}%`,
                            backgroundColor: PROGRESS_COLOR_THUMBNAILS,
                        },
                    ]}/>
                </View>)}
              {scanProgress.phase === 'enriching' && (<View style={styles.scanProgressTrack}>
                  <View style={[
                        styles.scanProgressFill,
                        {
                            width: `${Math.round((scanProgress.metadataDone / Math.max(1, scanProgress.metadataTotal)) * 100)}%`,
          ...
          <ThemedView style={styles.breadcrumbRow}>
            {navStack.length > 0 && (<TouchableOpacity onPress={navigateBack} style={styles.backButton}>
          ...
          <FlashList data={displayItems} keyExtractor={(item) => item.key} numColumns={numColumns} extraData={`${editMode}|${pressedKey ?? ''}`} renderItem={({ item }) => {
                ...
                const handlePress = isFolder
                    ...
                    : () => {
                        ...
                        openMediaInExternalApp(item.mediaObject.path, item.mediaObject.filename).catch((e) => {
                ...
                const isEditableInEditMode = editMode && (item.mediaType === 'show' ||
                ...
                const handlePressOut = isEditableInEditMode
                    ...
                    : hasBothImages
                        ? () => setPressedKey(null)
                        : undefined;
                return (<TouchableOpacity onPress={handlePress} onLongPress={handleLongPress} onPressOut={handlePressOut} style={[styles.card, { width: cardWidth }]}>
                  <View style={[styles.thumbnailBox, { height: thumbnailHeight }]}>
                    {displaySource ? (<Image source={displaySource} style={styles.thumbnailImage} contentFit={viewOrientation === 'poster' && isShowingVideoThumbnail ? 'contain' : 'cover'}/>) : (<View style={styles.thumbnailPlaceholder}>
                        <ThemedText style={styles.placeholderIcon}>
                          {isFolder ? '📁' : '🎬'}
                        </ThemedText>
                      </View>)}
                    {/* Edit mode indicator overlay */}
                    {isEditableInEditMode && (<View style={styles.editOverlay}>
        ...
        </ThemedView>) : isScanning ? (<ThemedView style={styles.stepContainer}>
          ...
          {mediaSources.length > 0 ? (<ThemedText>
