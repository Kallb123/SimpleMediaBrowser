...
import { addMediaSource } from '@/store/settingsReducer';
 ...
 */
export function AddMediaSource({ onAdded }) {
    ...
    const pickDirectory = useCallback(async () => {
        ...
        const source = { uri, contentType: selectedType };
    ...
    return (<View style={styles.row}>
      <SelectDropdown ref={dropdownRef} data={MEDIA_TYPE_OPTIONS} defaultValue={MEDIA_TYPE_OPTIONS[0]} onSelect={(item) => setSelectedType(item.id)} renderButton={(selectedItem, isOpened) => (<View style={[styles.dropdownButton, { backgroundColor: dropdownBg }]}>
            <Text style={[styles.dropdownButtonText, { color: theme.text }]}>
              {selectedItem ? selectedItem.label : 'Type…'}
            </Text>
            <Text>{isOpened ? '🔼' : '🔽'}</Text>
          </View>)} renderItem={(item, _index, isSelected) => (<View style={[
                styles.dropdownItem,
          ...
          </View>)} showsVerticalScrollIndicator={false} dropdownStyle={[styles.dropdownMenu, { backgroundColor: dropdownBg }]}/>
